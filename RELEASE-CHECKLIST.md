...
| 1c | `levels/r-III.json` no longer describes the shipped pack | The ten levels that went live are the hand-tuned versions, and the draft still holds the earlier geometry with its recorded answers — so `verify:levels` is replaying levels that no longer exist. Either re-export the draft from the live rows and find fresh answers for it, or delete it; leaving it is the drift the snapshot rule exists to prevent. | Both |
