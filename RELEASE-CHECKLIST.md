...
| 13 | Verify RLS on every table | Confirm players can only write their own rows and only the admin account can write override tables. Note `total_stars` is still self-writable — that is a leaderboard-forgery surface, not a payments one, but it is the same class of hole as 13b was. | You |
