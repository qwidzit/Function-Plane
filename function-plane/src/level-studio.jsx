...
const goalOut = s => { const t = String(s).trim(); return t && isFinite(Number(t)) ? Number(t) : null; };
...
function fileToLevel(doc) {
  ...
  window.FP_PARAMS = {};
  const equations = (Array.isArray(doc.equations) ? doc.equations : [])
    .filter(e => e && typeof e.expr === 'string' && e.expr.trim())
    .map((e, i) => {
      const segs = (Array.isArray(e.domain) ? e.domain : [])
        .filter(d => isFinite(d?.xMin) && isFinite(d?.xMax))
        .map(d => ({ xMin: Number(d.xMin), xMax: Number(d.xMax) }));
      const sx = Number(e.shift?.x) || 0, sy = Number(e.shift?.y) || 0;
      const shift = sx || sy ? { x: sx, y: sy } : null;
      return {
        id: i + 1, expr: e.expr, ...parseEquation(e.expr, shift), shift,
  ...
  return {
    ...
    objects: (Array.isArray(doc.objects) ? doc.objects : [])
      .filter(o => o && FP_OBJECTS.KINDS[o.kind]).map(o => ({ ...o })),
    equations,
...
function LevelStudio({ mode = 'sandbox', pack, levelIndex, onBack, onSaved, density = 'comfortable', settings }) {
  ...
  const exportLevel = () => {
    const doc  = levelToFile({ name, ball, stars, objects, equations, scoreGoal, eqGoal, materials, hint, explain });
    const slug = (doc.name || (admin ? `${pack.id}-${levelIndex + 1}` : 'sandbox'))
    ...
    setMsg({ text: `Exported ${slug}.json` });
  };
  ...
  const importLevel = file => {
    ...
    file.text().then(txt => {
      ...
      setName(next.name);
      setBall(next.ball); setStars(next.stars); setObjects(next.objects);
      setEquations(next.equations);
      setScoreGoal(next.scoreGoal); setEqGoal(next.eqGoal);
      setMaterials(next.materials); setHint(next.hint); setExplain(next.explain);
      setSelected(null); setTrail(null);
      ...
      setMsg({ text: `Imported ${file.name}` });
    }).catch(e => setMsg({ text: e instanceof SyntaxError ? 'That file is not JSON' : e.message, bad: true }));
  };
  ...
  const levelTab = {
    ...
    content: (
      <div style={{ padding: '4px 14px 12px' }}>
        <StudioField label="Level name" value={name} onChange={setName}
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <textarea value={hint} onChange={e => setHint(e.target.value)} rows={2}
            ...
            style={{
              ...
              border: '1px solid var(--lv-line)', color: 'var(--fp-ink)',
              fontSize: 13, lineHeight: 1.5, outline: 'none', fontFamily: 'inherit',
            }}/>
          <div style={{ fontSize: 11, color: 'var(--fp-ink-4)', marginTop: 4, lineHeight: 1.5 }}>
            What <em>kind</em> of function this level wants — never the numbers. Overrides
            the built-in hint for this level; leave empty to fall back to it.
          </div>
        </div>
        <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 10.5, color: 'var(--fp-ink-3)', marginBottom: 6, letterSpacing: '0.04em', textTransform: 'uppercase' }}>File</div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
          <button onClick={exportLevel} disabled={running} style={{ ...FILE_BTN, opacity: running ? 0.4 : 1 }}>Export</button>
          <button onClick={() => fileRef.current?.click()} disabled={running} style={{ ...FILE_BTN, opacity: running ? 0.4 : 1 }}>Import</button>
        </div>
        <input ref={fileRef} type="file" accept="application/json,.json" style={{ display: 'none' }}
          onChange={e => { importLevel(e.target.files?.[0]); e.target.value = ''; }}/>
        <div style={{ fontSize: 11, color: 'var(--fp-ink-4)', marginTop: 4, marginBottom: 12, lineHeight: 1.5 }}>
          The spawn, the stars, the objects, every equation and the goals, as one JSON file.
          Import replaces the whole board{admin ? ' — it does not save; the button below does' : ''}.
        </div>
        {admin && (
        ...
        {msg && <div style={{ marginTop: 8, fontSize: 12, textAlign: 'center', color: msg.bad ? '#e34' : 'var(--fp-accent)' }}>{msg.text}</div>}
      </div>
    ),
  };
...
window.FP_LEVEL_FILE = { LEVEL_FILE, levelToFile, fileToLevel };
