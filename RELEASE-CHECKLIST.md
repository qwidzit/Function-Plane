...
|---|---|---|---|
| 1 | Author the remaining 40 levels | Pack III (*Weightless*) is drafted — see `levels/r-III.json`, thirty answers replayed by `npm run verify:levels`. **Nothing is written to Supabase yet**, so players still get the placeholder until those rows and a `pack_overrides` row naming r-III *Weightless* exist. Packs IV, V, Linear and Trig have no level data at all. `npm test` prints live coverage. | You |
