...
|---|---|---|---|
| 1 | Author the remaining 30 levels | Packs III (*Weightless*) and Geometry (`s-qua`) are both **live** in `level_overrides` and baked into the snapshot. Packs IV, V, Linear and Trig have no level data at all. `npm test` prints live coverage. | You |
| 1d | **Only Linear and Geometry are visible right now** | Every Roman pack and Trigonometry are `is_hidden` in `pack_overrides`, so a player sees two packs and twenty levels. Geometry gates at 50 total stars in `SPECIAL_UNLOCK_STARS`, and the only stars now reachable are Linear's thirty placeholders — so a non-admin still cannot open it. Admins bypass every pack lock, so this is invisible while you are testing. Unhide the Roman packs (or author Linear) before anyone else plays. | You |
...
| 1b | The shape pack's ten shapes | Decided. Level *n* holds *n* stars: 1 a single star, 2 a line, 3 a 60-60-60 triangle, 4 a square, 5 a pentagon, 6 the hexagonal silhouette of a cube, 7 a heptagon, 8 three squares in a row, 9 a circle of nine, 10 the ten vertices of a five-pointed star. The studio's cap is raised from 8 to 10 to match. Closed shapes need Pack III's zero-gravity and rubber to lap, which is why the pack gates at 120 stars. | Both |
| 2 | Re-tune level goals | Score and equation goals across all 90 levels, once the levels exist. **Stars are now awarded individually** — clearing, the score goal and the equation goal each light their own, so a run that beats the equation goal but misses the score goal earns 2 rather than 3. `SPECIAL_UNLOCK_STARS` is now `lin 0 · trig 40 · flip 80 · qua 120`, ordered by what each pack depends on; the Roman-pack threshold stays at 27/30. | You |
