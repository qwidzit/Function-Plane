...
| 11e | Apply the level-hints migration | `supabase/migrations/20260916_level_hints.sql` adds `level_overrides.hint` — additive, one column. Until it is applied the built-in hints still show; only editing a hint in the studio needs it, and saving a level with one set fails until it is run. | You |
