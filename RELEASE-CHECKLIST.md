...
|---|---|---|---|
| 1 | Author the remaining 30 levels | Packs I, II, III (*Weightless*) and Geometry are **live** — forty authored levels. **Packs IV (*Attraction*), V (*Mastery*), Linear and Trigonometry have no level data at all** and repeat one placeholder; Linear and Trig are the two a player can currently reach, and Linear is visible. `npm test` prints live coverage. | You |
...
| 3b | Write the remaining level hints | Every level has a **Hint** button on the plane; `LEVEL_HINTS` in `src/data.jsx` carries packs I and II and every other level answers "No hint". A hint is one short thing and never the numbers: the family the level was built around (`Constant`, `Linear`, `Quadratic`), or, where the answer is several curves, the count (`Use 2 functions`). **It follows from the level's own goals** — `score_goal − 20 × eq_goal` is the complexity the author left room for, so 0 is a constant, 10 a line, 20 a parabola — which means writing one is bookkeeping, not judgement, once the level exists. Add them as the levels are authored (item 1), in that table or per level in the studio's **Level** tab, which writes `level_overrides.hint` and wins over it. | You |
