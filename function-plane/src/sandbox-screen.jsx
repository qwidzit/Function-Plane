...
const { useState: useSB, useRef: useSBR, useEffect: useSBE } = React;

const SB_START  = { x: -4, y: 5 };
const SB_STARS  = [{ x: -2, y: 2 }, { x: 0, y: 0 }, { x: 2, y: -2 }];
const SB_LIMIT  = 30;   // seconds of sim before a run stops on its own

function SandboxScreen({ onBack, density = 'comfortable', settings }) {
  ...
  useSBE(() => { if (!running) resetSim(); }, [ball, stars, running]);
  ...
  const selectedPos = selected === 'ball'
    ? ball
    : (selected ? stars[Number(selected.slice(5))] : null);

  const setCoord = (axis, value) => {
    const n = Number(value);
    if (!isFinite(n) || !selected) return;
    moveObject(selected, { ...selectedPos, [axis]: Math.max(-40, Math.min(40, n)) });
  };
  ...
  const handlePlay = () => {
    ...
    physRef.current = {
      ...
      bounced: false, justCollected: false,
  ...
  useSBE(() => {
    ...
    const { TICK_DT, SUB_STEPS, MAX_TICKS, BALL_R, FALL_LIMIT } = window.SIM;
    ...
    <div className="fp-screen" style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, textAlign: 'center' }}>
          ...
          }}>{running ? `${elapsed.toFixed(1)}s` : 'Free play'}</div>
        </div>
      ...
      {/* Object picker — the only way to choose what you're moving */}
      <div style={{
        display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap',
        padding: `0 ${padX}px 8px`, flex: '0 0 auto', background: 'var(--lv-bg)',
      }}>
        <ObjChip label="Spawn" active={selected === 'ball'} disabled={running}
          onClick={() => setSelected(selected === 'ball' ? null : 'ball')}/>
        {stars.map((_, i) => (
          <ObjChip key={i} label={`Star ${i + 1}`} active={selected === `star-${i}`} disabled={running}
      ...
      <div style={{
        ...
        {!selected && !running && (
          <div style={{
            ...
            Pick an object above, or drag one on the plane
          </div>
        )}
      </div>
      ...
      {selectedPos && !running && (
        <div style={{
          ...
          }}>{selected === 'ball' ? 'Spawn' : `Star ${Number(selected.slice(5)) + 1}`}</span>
          <CoordField axis="x" value={selectedPos.x} onChange={setCoord}/>
          <CoordField axis="y" value={selectedPos.y} onChange={setCoord}/>
        </div>
      )}
...
function ObjChip({ label, active, disabled, onClick }) {
  return (
...
function CoordField({ axis, value, onChange }) {
  ...
  useSBE(() => { setDraft(String(value)); }, [value]);
  return (
