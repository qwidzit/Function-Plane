...
| 41 | **Leaked-password protection is Pro-plan only** (the project is on Free). Either upgrade and switch it on — Authentication ▸ Sign In / Providers ▸ Email ▸ *Prevent use of leaked passwords* — or, on Free, raise *Minimum password length* to 8 there and match the app, which still accepts 6 (`register` in `accounts.js`, and the account screen hint) |
