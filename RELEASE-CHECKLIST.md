...
| 2b | **Three pack II levels may be mis-tuned** | `r-II` 3 *Wind tunnel*, 6 *Above the wall* and 7 *Trickshot* all carry `score_goal` 30, which is one linear — but a grid search over ~31k lines, plain and cut to one side, clears none of them. Wind tunnel and Trickshot are cleared by a parabola at 40 pts, over their own goal; Above the wall by nothing in ~36k candidates. The search does not try two-sided domain windows, trig, implicit or parameterised curves, so this is a strong signal rather than a proof — but the second star looks unreachable on all three. Wind tunnel behaves identically with the fan at its old position, so the September fan move is not the cause. | You |
