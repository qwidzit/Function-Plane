...
|---|---|---|---|
| 1 | Author the remaining 20 levels | Packs I, II, III (*Weightless*), IV (*Attraction*) and Geometry are **live** — fifty authored levels, `npm test` prints the coverage. **Pack V (*Mastery*) and Linear still have no level data** and repeat the one placeholder; Linear is visible and always unlocked, so it is the one a player meets first. Trigonometry is unauthored too but hidden, so it is not reachable. Note Geometry is authored but not finishable — item 1e. | You |
| 1e | **Geometry III–X cannot be finished** | **Blocking, and reachable now** — the pack is visible and gates at 50 stars, which Roman I–III alone cover. Only *One* and *Two* clear: a sweep of ~1000 single curves (lines, both parabola orientations, circles, ellipses) plus a greedy search adding a second and third curve gets *Three* 2/3, *Four* 2/4, *Five* 2/5, *Six* 2/6, *Seven* 4/7, *Eight* 4/8, *Nine* 7/9 and *Ten* 4/10, and every one of those ends in a timeout or off the edge of the world. The cause is the machinery, not the geometry: the pack sets `materials: false` and carries no objects, and a ball under plain gravity cannot travel round a ring of stars — it rides the *outside* of a circle and slides off. Rubber alone does not fix it, and nor does a zero-gravity box, because these levels spawn the ball inside the box and a zone cannot start a still ball moving. What each level needs is a spawn outside the zone, a way in, and the bounce toggle on. Until then eight of its ten hints are deliberately unwritten (item 3b) — greedy search is not proof of unsolvability, but nothing found an answer. | Both |
...
| 2 | Re-tune level goals | Score and equation goals across all 90 levels, once the levels exist. **Stars are now awarded individually** — clearing, the score goal and the equation goal each light their own, so a run that beats the equation goal but misses the score goal earns 2 rather than 3. `SPECIAL_UNLOCK_STARS` is now `lin 0 · trig 40 · qua 50 · flip 80`, ordered by what each pack depends on; the Roman-pack threshold stays at 27/30. | You |
...
| 3 | ~~Set the `stars_200` threshold~~ | **Decided: leave it at 200.** The maximum is 270 rather than 210 once all nine packs ship, so 200 is 74% of every star in the game — a real grind, but reachable. Editable in the admin panel if play says otherwise. | You |
| 3b | Write the remaining level hints | Every level has a **Hint** button on the plane. **Packs I–IV are complete**: `LEVEL_HINTS` in `src/data.jsx` carries I–III and `level_overrides.hint` carries Attraction's ten. **Geometry is the only gap, and it is blocked on item 1e** — a hint follows from `score_goal - 20 x eq_goal`, which is bookkeeping only once the level has an answer, and eight of its ten have none; *One* and *Two* are hinted, the rest wait. `r-III` X is deliberately blank as that pack's finale. Pack V and Linear get theirs as they are authored (item 1). | You |
| 4 | Regenerate the data snapshot | `npm run snapshot:data` bakes the finished Supabase level data into the app. Must be the last content step — and it is now also what carries authored **objects** (fans, zero-gravity zones, wells, hazards), the per-level bounce toggle, and each level's `explain` and `hint` into offline boots. It reads with `GET` only, so it works from anywhere the game itself works — unlike saving a level, which is a `POST` and is what vanishes on some networks ([`NETWORK-ACCESS.md`](./NETWORK-ACCESS.md)). | You |
...
| 10b | **Decided: one lifetime unlock at €4.90** | Was monthly/annual/lifetime. `is_premium` is a boolean with no expiry, so the schema cannot express a lapsed subscription; a one-time unlock also drops grace periods, billing retry and subscription restore from the work. The screen, the config and the tests are updated — and as of item 17 so are both Terms, which still carried a forward-looking clause about auto-renewal. A test now holds that line. | Claude |
...
| # | Item | Description | Who |
|---|---|---|---|
| 17 | Publish the legal-text corrections | **Still not live.** `legal/privacy.html`, `legal/delete-account.html` and now `legal/terms.html` are corrected and match the in-app copies; the live site still serves the old ones. Terms section 5 no longer describes auto-renewing subscriptions — premium is one lifetime unlock, so a recurring charge is a product this game will never have — and its effective date is 17 September 2026 in both copies. `npm test` now fails if the two Terms disagree on that date or if either one describes a subscription. **Left to you:** hand `legal/WEBSITE-AGENT-PROMPT-PRIVACY-UPDATE.md` plus all three HTML files to the website agent, then fetch `/privacy.html`, `/delete-account.html` and `/terms.html` and compare the served HTML against `legal/` — a previous deploy returned the homepage with a 200, so a status code proves nothing. | Both |
