...
| 3b | Write the remaining level hints | Every level has a **Hint** button on the plane; `LEVEL_HINTS` in `src/data.jsx` carries the ten for pack I and every other level currently answers "No hint". A hint is one short thing and never the numbers: the family the level was built around (`Linear`, `Quadratic`), or, where the answer is several curves, the count (`Use 3 functions`). Add them as the levels are authored (item 1), either in that table or per level in the studio's **Level** tab, which writes `level_overrides.hint` and wins over the table. | You |
