...
| 1d | **Only Linear and Geometry are visible right now** | Every Roman pack and Trigonometry are `is_hidden` in `pack_overrides`, so a player sees two packs and twenty levels. Geometry still gates at 120 total stars in `SPECIAL_UNLOCK_STARS`, and the only stars now reachable are Linear's thirty placeholders — so a non-admin cannot open it. Admins bypass every pack lock, so this is invisible while you are testing. Unhide the rest, or set `s-qua` to 0, before anyone else plays. | You |
