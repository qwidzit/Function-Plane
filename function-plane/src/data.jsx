...
function getLevelData(packId, levelIndex) {
  ...
  const ov   = window.FP_LEVEL_OVERRIDES?.[`${packId}-${levelIndex}`];
  if (!ov) return { ...base, preplaced: [], objects: [], outline: [], materials: false, explain: null, hint };
  return {
...
function computePackLocked(progress, pack) {
  if (hasFullAccess()) return { locked: false };
...
function findContinuePoint(progress) {
  ...
  for (const pack of [...ROMAN_PACKS, ...SPECIAL_PACKS]) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (s === null || s === -1 || s === 0) return { pack, levelIndex: i };
    }
  }
...
Object.assign(window, {
  ...
  computePackLocked, hasFullAccess,
  findContinuePoint,
