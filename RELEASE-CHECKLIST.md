...
|---|---|---|---|
| 1 | Author the remaining 40 levels | Pack III (*Weightless*) is **live**: ten rows in `level_overrides`, a `pack_overrides` row naming r-III *Weightless*, and both baked into the snapshot. Packs IV, V, Linear and Trig have no level data at all; Geometry is drafted in `levels/s-qua.json` as shapes only. `npm test` prints live coverage. | You |
| 1c | ~~`levels/r-III.json` no longer describes the shipped pack~~ | **Done — deleted.** A `levels/*.json` is a draft, not a game file: nothing in `function-plane/` reads one, and only `npm run verify:levels` does. Pack III's levels were authored and tuned in the studio rather than from a draft, so the live rows and `overrides-snapshot.js` are the record; keeping a stale draft only meant `verify:levels` reporting green on levels that no longer exist. `levels/s-qua.json` stays because nothing has shipped from it yet and it is still the only written record of Geometry's ten shapes. | Both |
