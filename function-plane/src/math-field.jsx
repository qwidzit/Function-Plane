...
const { useState: useMF, useRef: useMR, useEffect: useME, useLayoutEffect: useMLE } = React;
...
const MF_FNS = ['arcsin','arccos','arctan','asin','acos','atan','sin','cos',
  'tan','sqrt','abs','log','ln','exp','floor','ceil','round','max','min',
  'pow','sgn','sum','deriv','integ'];

const MF_BR = {
  paren: { open: '(', close: ')', text: s => `(${s})` },
  abs:   { open: '|', close: '|', text: s => `abs(${s})` },
  floor: { open: '⌊', close: '⌋', text: s => `floor(${s})` },
  ceil:  { open: '⌈', close: '⌉', text: s => `ceil(${s})` },
};
// What a typed bracket character is: its kind, and which side it is. A pipe
// is either side.
const MF_BR_CH = {
  '(': ['paren', 'L'], ')': ['paren', 'R'], '|': ['abs', null],
...
class MBlock {
  constructor(parent) { this.id = ++mfSeq; this.nodes = []; this.parent = parent || null; }
  insert(i, nodes) { for (const n of nodes) n.parent = this; this.nodes.splice(i, 0, ...nodes); }
  remove(i, count) { return this.nodes.splice(i, count); }
  indexOf(n) { return this.nodes.indexOf(n); }
  get length() { return this.nodes.length; }
  isEmpty() { return this.nodes.length === 0; }
}

const mkBlock = (owner, nodes) => { const b = new MBlock(owner); b.insert(0, nodes || []); return b; };
const mkCh   = c => ({ id: ++mfSeq, t: 'ch', c });
const mkFrac = (num, den) => { const n = { id: ++mfSeq, t: 'frac' }; n.num = mkBlock(n, num); n.den = mkBlock(n, den); return n; };
const mkSup  = body => { const n = { id: ++mfSeq, t: 'sup' }; n.body = mkBlock(n, body); return n; };
const mkSqrt = body => { const n = { id: ++mfSeq, t: 'sqrt' }; n.body = mkBlock(n, body); return n; };
// ghost: null (both sides drawn), 'L' or 'R' (that side is the faint one).
const mkBr   = (kind, body, ghost) => { const n = { id: ++mfSeq, t: 'br', kind, ghost }; n.body = mkBlock(n, body); return n; };
...
function textOf(block) { return block.nodes.map(nodeText).join(''); }

function nodeText(n) {
  switch (n.t) {
    case 'ch':   return n.c;
    case 'frac': return `${wrapText(n.num)}/${wrapText(n.den)}`;
    case 'sup':  return `^${wrapText(n.body)}`;
    case 'sqrt': return `sqrt(${textOf(n.body)})`;
    case 'br':   return MF_BR[n.kind].text(textOf(n.body));
  }
}
...
function isPrimary(nodes) {
  ...
  const last = nodes[nodes.length - 1], head = nodes.slice(0, -1);
  return last.t === 'br' && last.kind === 'paren' && head.every(n => isCh(n, /[a-zA-Z]/))
    && MF_FNS.includes(head.map(n => n.c).join(''));
}
...
function mfTokens(src) {
  ...
  while (i < src.length) {
    ...
    if (/[a-zA-Z]/.test(c)) {
      ...
      out.push(run === 'pi' ? { t: 'other', v: 'π' } : { t: 'name', v: run }); i = j; continue;
    }
...
function parseText(src) {
  ...
  function primary() {
    const tk = peek();
    if (!tk) return [];
    if (tk.t === 'num') { p++; return [...tk.v].map(mkCh); }
    if (tk.t === 'other') { p++; return [mkCh(tk.v)]; }
  ...
  function term() {
    const out = [];
    while (opIs('-') || opIs('+')) { out.push(mkCh(peek().v)); p++; }
    out.push(...primary());
    while (opIs('^')) { p++; out.push(mkSup(term())); }
  ...
  function seq(inGroup) {
    ...
    for (;;) {
      const tk = peek();
      if (!tk) break;
      if (tk.t === ')') { if (inGroup) break; p++; out.push(mkCh(')')); continue; }
      if (opIs('/')) {
      ...
      if (opIs('^')) { p++; out.push(mkSup(term())); continue; }
      if (tk.t === 'op' || tk.t === ',') { p++; out.push(mkCh(tk.v)); continue; }
      const prim = primary();
      if (prim.length) { out.push(...prim); continue; }
      p++;
    }
    return out;
  }
...
class MathField {
  ...
  write(str) { for (const ch of str) this.type(ch); this.changed(); }
  typed(ch) { this.type(ch); this.changed(); }
  type(ch) {
    if (ch === ' ') return;
    if (ch === '×' || ch === '·') ch = '*';
    if (ch === '−') ch = '-';
  ...
  exitSup() {
    ...
    this.cur = { blk: s.parent, i: s.parent.indexOf(s) + 1 };
  }
  ...
  bracket(ch, quiet) {
    ...
    let m = null;
    if (!typedSide) {
      if (oneSided(blk.nodes[i], 'R')) m = blk.nodes[i];
      else if (oneSided(blk.nodes[i - 1], 'L')) m = blk.nodes[i - 1];
      else if (oneSided(enclosing)) m = enclosing;
    } else {
      const solid = typedSide === 'L' ? 'R' : 'L';
      const beside = typedSide === 'L' ? blk.nodes[i] : blk.nodes[i - 1];
      if (oneSided(beside, solid)) m = beside;
      else if (oneSided(enclosing, solid)) m = enclosing;
    }
    if (m) {
      const side = m.ghost;
      m.ghost = null;
      if (m === enclosing) {
        ...
        const pb = m.parent, at = pb.indexOf(m);
        if (side === 'R') pb.insert(at + 1, blk.remove(i, blk.length - i));
        else pb.insert(at, blk.remove(0, i));
      }
      this.cur = side === 'L' ? { blk: m.body, i: 0 } : { blk: m.parent, i: m.parent.indexOf(m) + 1 };
    } else if ((typedSide || 'L') === 'L') {
      const n = mkBr(kind, blk.remove(i, blk.length - i), 'R');
      blk.insert(i, [n]);
      this.cur = { blk: n.body, i: 0 };
    } else {
  ...
  unwrap(n) {
    const pb = n.parent, at = pb.indexOf(n);
  ...
  deleteSide(n, side, outward) {
    ...
    const body = n.body;
    const beside = () => {
      if (sib) { const b = sib.parent; this.cur = { blk: b, i: b.indexOf(sib) + (side === 'R' ? 0 : 1) }; }
      else this.cur = { blk: pb, i: side === 'R' ? pb.length : 0 };
    };
    if (n.ghost && side === solidOf(n)) { this.unwrap(n); beside(); return; }
    ...
    if (enc && enc.kind === n.kind && enc.ghost && solidOf(enc) === side) { enc.ghost = null; this.unwrap(enc); }
    else if (outward && wasSolid) { this.unwrap(n); beside(); return; }
    if (sib) {
      const b = sib.parent, k = b.indexOf(sib);
      if (side === 'R') body.insert(body.length, b.remove(k, b.length - k));
      else body.insert(0, b.remove(0, k + 1));
      beside();
    } else if (outward) {
      this.cur = { blk: n.parent, i: n.parent.indexOf(n) + (side === 'R' ? 1 : 0) };
    } else {
      this.cur = { blk: body, i: side === 'R' ? body.length : 0 };
    }
  }
  ...
  deleteDir(dir) {
    ...
    if (n) {
      ...
      if (n.t === 'br') this.deleteSide(n, dir === 'L' ? 'R' : 'L', false);
      else if (n.t === 'ch' || kidsOf(n).every(b => b.isEmpty())) { blk.remove(at, 1); this.cur = { blk, i: at }; }
      else {
        // Not empty: step into it instead, the way the caret would.
        const ks = kidsOf(n), b = dir === 'L' ? ks[ks.length - 1] : ks[0];
        this.cur = { blk: b, i: dir === 'L' ? b.length : 0 };
      }
    } else if (blk.parent) {
      const p = blk.parent;
      if (p.t === 'br') this.deleteSide(p, dir, true);
      else this.spill(p);
    }
    this.changed();
  }
  backspace() { this.deleteDir('L'); }
  ...
  moveDir(dir) {
    ...
    if (n) {
      const ks = kidsOf(n);
      if (!ks.length) this.cur = { blk, i: dir === 'L' ? i - 1 : i + 1 };
      else { const b = dir === 'L' ? ks[ks.length - 1] : ks[0]; this.cur = { blk: b, i: dir === 'L' ? b.length : 0 }; }
    } else if (blk.parent) {
      const p = blk.parent, ks = kidsOf(p), k = ks.indexOf(blk);
      const next = ks[dir === 'L' ? k - 1 : k + 1];
      if (next) this.cur = { blk: next, i: dir === 'L' ? next.length : 0 };
      else this.cur = { blk: p.parent, i: p.parent.indexOf(p) + (dir === 'L' ? 0 : 1) };
    }
    this.changed();
  }
  left()  { this.moveDir('L'); }
  ...
  upDown(dir, x, rectOf) {
    ...
    if (r && into(r)) this.cur = { blk: into(r), i: 0 };
    else if (l && into(l)) this.cur = { blk: into(l), i: into(l).length };
    else {
      for (let b = blk; b.parent; b = b.parent.parent) {
        ...
        if (n.t === 'frac') {
          ...
          if (other) {
            if (x != null && rectOf) this.seekBlock(other, x, rectOf);
            else this.cur = { blk: other, i: other.length };
            break;
          }
        } else if (n.t === 'sup' && dir === 'down') {
          const pb = n.parent, at = pb.indexOf(n);
          this.cur = { blk: pb, i: b === blk && i === b.length ? at + 1 : at };
          break;
        }
      }
    }
  ...
  seekBlock(b, x, rectOf) {
    ...
    if (!last || rectOf(last).right < x) { this.cur = { blk: b, i: b.length }; return; }
    if (x < rectOf(b.nodes[0]).left) { this.cur = { blk: b, i: 0 }; return; }
    let k = b.length - 1;
    while (k > 0 && x < rectOf(b.nodes[k]).left) k--;
    this.seekNode(b.nodes[k], x, rectOf);
  }
  seekNode(n, x, rectOf) {
    const r = rectOf(n), pb = n.parent, at = pb.indexOf(n), ks = kidsOf(n);
    if (!ks.length) { this.cur = { blk: pb, i: at + (x - r.left < r.right - x ? 0 : 1) }; return; }
    if (x < r.left) { this.cur = { blk: pb, i: at }; return; }
    if (x > r.right) { this.cur = { blk: pb, i: at + 1 }; return; }
    let leftBound = r.left;
    for (let k = 0; k < ks.length; k++) {
      const b = ks[k], br = rectOf(b);
      if (x < br.left) {
        if (x - leftBound < br.left - x) this.cur = k > 0 ? { blk: ks[k - 1], i: ks[k - 1].length } : { blk: pb, i: at };
        else this.cur = { blk: b, i: 0 };
        return;
      }
      if (x > br.right) {
        if (k < ks.length - 1) { leftBound = br.right; continue; }
        this.cur = r.right - x < x - br.right ? { blk: pb, i: at + 1 } : { blk: b, i: b.length };
        return;
      }
  ...
  lookup(id, block) {
    const walk = b => {
      ...
      for (const n of b.nodes) {
        ...
        for (const k of kidsOf(n)) { const hit = walk(k); if (hit) return hit; }
      }
      return null;
    };
...
const mfCaret = () => <span key="caret" className="fp-caret" style={{
...
function renderBlock(b, cur, sel) {
  ...
  if (b.isEmpty()) {
    ...
    if (!b.parent) return <span data-b={b.id} style={{ ...MF_ROW, alignSelf: 'stretch', flex: 1 }}>{here ? mfCaret() : null}</span>;
    return (
      <span data-b={b.id} style={{
        ...MF_ROW, justifyContent: 'center', minWidth: '0.66em', height: '1.05em', margin: '0 1px',
        borderRadius: 2, border: '1px dashed color-mix(in srgb, currentColor 45%, transparent)',
      }}>{here ? mfCaret() : null}</span>
    );
  }
  const kids = [];
  b.nodes.forEach((n, k) => {
...
function MathFieldView({ field, focused, disabled, onFocus, onBlur, onDone, placeholder, color }) {
  const inputRef = useMR(null);
  const boxRef = useMR(null);
  useME(() => { field.input = inputRef.current; return () => { if (field.input === inputRef.current) field.input = null; }; }, [field]);
  ...
  useMLE(() => {
    const box = boxRef.current, c = box?.querySelector('.fp-caret');
    if (!c) return;
    const r = c.getBoundingClientRect(), b = box.getBoundingClientRect();
    if (r.left < b.left + 8) box.scrollLeft -= b.left + 8 - r.left;
  ...
  const onKey = e => {
    ...
    else if (k === 'Home') field.home();
    else if (k === 'End') field.end();
    else if (k === 'Enter') onDone?.();
    else if (k.length === 1) field.typed(k);
    else return;
    e.preventDefault();
  };
    ...
    <div ref={boxRef} className="fp-scroll" onPointerDown={onDown} onPointerMove={onMove}
      ...
      <input ref={inputRef} inputMode="none" spellCheck={false} autoComplete="off" disabled={disabled}
        ...
        style={{ position: 'absolute', left: 0, top: 0, width: 1, height: 1, opacity: 0,
          border: 0, padding: 0, pointerEvents: 'none' }}/>
    </div>
  );
}
...
window.FP_MATH = { parseText, textOf, MF_FNS };
