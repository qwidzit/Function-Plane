...
Guidance for working in the Function Plane repository. This file is the single
merged source of onboarding notes for this repo (it used to be split across
CLAUDE.md and AI.md — they said overlapping and, in places, contradictory
things after the physics/data/classifier rewrites, so this replaces both).
...
- [ ] **Reset-password deep link** so the email link opens the app: either a
      custom URL scheme (`app.functionplane://auth/reset`, registered as an
      Android `<intent-filter>` + iOS `LSApplicationQueriesSchemes`) or App
      Links/Universal Links via `assetlinks.json` /
      `apple-app-site-association` on a real https domain (recommended).
      Then update `redirectTo` in the password-reset call and add the same
      URL under Supabase → Authentication → URL Configuration → Redirect
      URLs.
- Done already: `LEGAL_WEBSITE` set to the public site URL; Android hardware
  back button (`@capacitor/app`); haptics removed.

### Housekeeping

- Stray files `main` (empty) and `test.md` (leftover push-permission test)
  are still committed at the repo root and can be deleted.
...
- Hand-edited a compiled `.js` for a "one-line fix" without checking it
  ...
  Normalize `\uXXXX`/`\xXX` unicode escapes before diffing (Babel-version
  churn, see above), don't assume a raw string diff.
...
- Star/pack totals look wrong → check whether hidden packs are being counted
  ...
  see "Level & pack data" above.
- A themed pack lets through an equation it shouldn't (or blocks one it
  should allow) → the classifier is AST-based now; check `detectClass()` in
...
- Read the header comments in `physics-engine.js` and `equation-classifier.js`
  — both are extensively documented at the top.
- Keep code minimal — remove unused paths rather than leaving them,
  no backwards-compat cruft, no haptics without being asked.
- Always re-run `npm run build:jsx` after `.jsx` changes AND bump `sw.js`'s
  cache version before declaring a change "done".
